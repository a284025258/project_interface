...
class AddProjectTestCase(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        cls.pdbc = PDBC()
        cls.fake = Faker()
        url = my_config.get('url', 'url') + 'user/login/'
        param = {'username': my_config.get('data', 'username'), 'password': my_config.get('data', 'password')}
        cls.response = http_request.request(url=url, method='post', data=param)
        cls.token = cls.response.json()['token']
    ...
    def test_add_project_case(self, case):
        ...
        # 准备参数
        # 动态参数替换
        a = self.fake.sentence()[:4] + '--项目'
        case.param = case.param.replace('*project_name*', self.fake.sentence()[:4] + '--项目')
        case.param = case.param.replace('*leader*', self.fake.name())
        case.param = case.param.replace('*tester*', self.fake.name())
        case.param = case.param.replace('*programmer*', self.fake.name())
        case.param = case.param.replace('*publish_app*', self.fake.sentence()[:4] + '--项目')
        case.param = case.param.replace('*desc*', self.fake.sentence())
        log.info(case.param)
        url = my_config.get('url', 'url') + case.url
        # 发生请求接口
        log.info(f'正在请求地址：{url}')
        # data_replace() 参数动态化处理 从配置文件读取 username 及 password 用于登录
        response = http_request.request(url, case.method, eval(data_replace(case.param)),
